...
int main(int argc, char *argv[]) {
  ...
  for (int i = 0; i < reconstructions.size(); i++) {
    ...
    LOG(INFO)
        ...
        << " to " << reconstruction_file;
    CHECK(theia::WriteReconstruction(*reconstructions[i], reconstruction_file))
        << "Could not write reconstruction to file.";
    CHECK(WritePlyFile(reconstruction_file + ".ply",
                       *reconstructions[i],
    ...
    theia::ColorizeReconstruction(FLAGS_images_directory,
                                  ...
                                  reconstructions[i]);
    LOG(INFO)
        << "Writing colorized reconstruction " << i
        << " to " << reconstruction_file_with_color;
    CHECK(theia::WriteReconstruction(*reconstructions[i],
                                     reconstruction_file_with_color));
    CHECK(WritePlyFile(reconstruction_file_with_color + ".ply",
                       *reconstructions[i],
                       FLAGS_min_num_observations_per_point))
        << "Could not write out PLY file with color properties.";
  }
}
